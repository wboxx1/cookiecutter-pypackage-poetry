...
def run_project_tests(dirpath):
    """Run baked project tests without requiring package installation."""
    env = os.environ.copy()
    env["PYTHONPATH"] = os.path.join(dirpath, "src")
    with inside_dir(dirpath):
        return subprocess.check_call([sys.executable, "-m", "pytest"], env=env)
...
def project_info(result):
    ...
    project_slug = os.listdir(src_path)[0]
...
def test_bake_with_defaults(cookies):
    with bake_in_temp_dir(cookies) as result:
        ...
        assert 'Justfile' in found_toplevel_files
        assert 'Makefile' not in found_toplevel_files
        assert 'make.ps1' not in found_toplevel_files
        assert '.travis.yml' not in found_toplevel_files
        assert 'appveyor.yml' not in found_toplevel_files
...
def test_bake_and_run_tests(cookies):
    with bake_in_temp_dir(cookies) as result:
        assert result.project.isdir()
        run_project_tests(str(result.project)) == 0
        print("test_bake_and_run_tests path", str(result.project))


def test_bake_withspecialchars_and_run_tests(cookies):
    """Ensure that a `full_name` with double quotes does not break metadata."""
...
def test_bake_with_apostrophe_and_run_tests(cookies):
    """Ensure that a `full_name` with apostrophes does not break metadata."""
...
def test_justfile(cookies):
    with bake_in_temp_dir(cookies) as result:
        justfile = result.project.join('Justfile').read()
        assert "poetry run ruff check src tests" in justfile
        assert "poetry run tox" in justfile
